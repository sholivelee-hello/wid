...
export function SourceIcon({ source, className }: SourceIconProps) {
  // 아이콘 크기는 호출부가 폰트크기(em 기준)로 정한다 — 래퍼 기본 14px,
  // 내부 svg/점은 em 비율로 따라간다. 상세 모달처럼 20px가 필요하면 호출부에서
  // `text-[20px]`만 주면 된다 (`[&_svg]` 오버라이드는 specificity가 불안정).
  const box = cn('inline-flex items-center justify-center flex-shrink-0 text-[14px]', className);

  if (source === 'slack') {
    return (
      <span className={box} aria-label="Slack에서 온 task" title="Slack">
        <svg viewBox="0 0 24 24" className="h-[1em] w-[1em]" aria-hidden role="img">
  ...
  return (
    <span className={box} aria-label="직접 입력한 task" title="직접 입력">
      <span aria-hidden className="inline-block h-[0.57em] w-[0.57em] rounded-full bg-primary" />
    </span>
  );
}
