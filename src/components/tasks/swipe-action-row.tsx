...
export function SwipeActionRow({
  ...
  const handleTouchEnd = () => {
    ...
    const th = threshold();
    const passed = Math.abs(dx) >= th;
    // 스와이프 모드였으면 이어지는 합성 click 1회 무시(상세 오발 방지).
    suppressClickRef.current = true;

    if (passed) {
      if (dx < 0 && onSwipeComplete) onSwipeComplete();
      else if (dx > 0 && onSwipePend) onSwipePend();
    }
  ...
  return (
    ...
    >
      {/* 힌트 레이어 — row 뒤 배경. 미는 방향에 따라 한쪽만 표시. 새 색 금지. */}
      {showCompleteHint && (
        <div
          aria-hidden
          className="absolute inset-0 flex items-center justify-end pr-5 rounded-md bg-primary/15"
        >
          <CheckCircle2
            className={cn(
      ...
      {showPendHint && (
        <div
          aria-hidden
          className="absolute inset-0 flex items-center justify-start pl-5 rounded-md bg-muted"
        >
          <PauseCircle
            className={cn(
              'h-5 w-5 transition-colors',
