...
- 전체 task 목록 fetch(`/api/tasks?deleted=false`)는 **렌더를 막지 않는 백그라운드 재검증**으로 강등 — prop 배열이 불완전한 페이지(예: 돌아보기의 월 범위 목록)에서도 관계가 정확하도록 진실원본 역할만 한다. (플랜 셀프리뷰에서 "완전 제거"보다 안전한 형태로 보정, 2026-06-05)
- 신선도: 백그라운드로 해당 task 1건도 재검증(fetch)해 stale하면 조용히 갱신.
